 ...
 */
export function validateStockOperationParams(
  operations: Array<{
    productId: string;
    quantity: number;
    size?: string;
...
export async function inventoryHealthCheck(): Promise<{
  ...
  try {
    ...
    if (expiredReservations > 50) {
      issues.push(
        `High number of expired reservations: ${expiredReservations}`,
      );
    }
    ...
    if (totalReservations > 1000) {
      issues.push(
        `Very high number of active reservations: ${totalReservations}`,
      );
    }
