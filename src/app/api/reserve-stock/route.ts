...
export async function POST(request: NextRequest) {
  try {
    const { operations, sessionId, expirationMinutes = 30 } = await request.json()

    // Validate request data
    if (!Array.isArray(operations) || !sessionId) {
      return NextResponse.json(
        { error: 'Invalid request data. Expected operations array and sessionId.' },
        { status: 400 }
      )
    }
    ...
    // Reserve stock
    const result = await reserveStock(operations as StockOperation[], sessionId, expirationMinutes)
