...
export async function POST(request: NextRequest) {
  try {
    ...
    if (product.stripePriceId) {
      console.log(
        `Product ${product._id} already has Stripe price ID: ${product.stripePriceId}`,
      );
      return NextResponse.json({
    ...
    if (isApparel && product.variants && product.variants.length > 0) {
      console.log(
        `Creating ${product.variants.length} variant prices for apparel product`,
      );
      ...
      for (const variant of product.variants) {
        ...
        console.log(
          `Created variant price: ${variantPrice.id} for size: ${variant.size}`,
        );
      }
