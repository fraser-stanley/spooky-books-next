...
export async function POST() {
  ...
  try {
    ...
    const totalFixed = results.variantKeys.fixed + 
...
async function fixVariantKeys(results: { processed: number; fixed: number; errors: string[] }) {
  try {
    ...
    console.log(`   ✅ Fixed ${results.fixed} products with missing variant keys`)
...
async function initializeReservedQuantities(results: { processed: number; fixed: number; errors: string[] }) {
  try {
    ...
    results.processed = products.length
    console.log(`   Found ${products.length} products to check for reservedQuantity fields`)
    ...
    for (const product of products) {
      ...
      if (product.variants && product.variants.length > 0) {
        product.variants.forEach((variant: { size: string; reservedQuantity?: number; stockQuantity?: number }, index: number) => {
          if (variant.reservedQuantity === undefined || variant.reservedQuantity === null) {
            transaction.patch(product._id, {
              setIfMissing: { [`variants[${index}].reservedQuantity`]: 0 }
            })
            needsUpdate = true
            console.log(`   Adding reservedQuantity to ${product.title} size ${variant.size}`)
          }
        })
      }
    ...
    console.log(`   ✅ Initialized reservedQuantity for ${results.fixed} products`)

  } catch (error) {
    const errorMsg = error instanceof Error ? error.message : 'Unknown error'
    results.errors.push(`Reserved quantities initialization failed: ${errorMsg}`)
    console.error('   ❌ Failed to initialize reserved quantities:', error)
  }
}

async function ensureStockQuantities(results: { processed: number; fixed: number; errors: string[] }) {
  try {
    ...
    results.processed = products.length
    console.log(`   Found ${products.length} products to check for stockQuantity fields`)
    ...
    for (const product of products) {
      ...
      if (product.variants && product.variants.length > 0) {
        product.variants.forEach((variant: { size: string; stockQuantity?: number }, index: number) => {
          if (variant.stockQuantity === undefined || variant.stockQuantity === null) {
            transaction.patch(product._id, {
              setIfMissing: { [`variants[${index}].stockQuantity`]: 0 }
            })
            needsUpdate = true
            console.log(`   Adding stockQuantity to ${product.title} size ${variant.size}`)
          }
        })
      }
...
async function updateHasSizes(results: { processed: number; fixed: number; errors: string[] }) {
  try {
    ...
    results.processed = products.length
    console.log(`   Found ${products.length} products to check for hasSizes field`)

    for (const product of products) {
      const isApparel = product.category?.title?.toLowerCase() === 'apparel'
      const actuallyHasSizes = isApparel && product.variants && product.variants.length > 0
      ...
      if (product.hasSizes !== actuallyHasSizes) {
        ...
        results.fixed++
        console.log(`   ✅ Updated hasSizes for ${product.title}: ${actuallyHasSizes}`)
      }
    }
...
export async function GET() {
  return NextResponse.json({
    ...
    description: 'Fixes all issues needed for existing products to work with the autonomous inventory system',
    fixes: [
    ...
    warning: 'This operation modifies your Sanity dataset. Backup recommended before running.'
  })
}