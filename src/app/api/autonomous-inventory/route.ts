...
async function handlePaymentCompleted(productId?: string) {
  console.log(
    `💳 Handling payment completion for product: ${productId || "unknown"}`,
  );
...
async function handleEmergencyCleanup() {
  ...
  try {
    ...
    if (productsWithIssues.length > 0) {
      ...
      actions.push(
        `negative-stock-reset-${productsWithIssues.length}-products`,
      );
    }
...
async function autoFixNegativeStock(product: {
  _id: string;
  title?: string;
  ...
  if (product.variants) {
    product.variants.forEach((variant, index: number) => {
      if ((variant.reservedQuantity || 0) > variant.stockQuantity) {
        transaction.patch(product._id, {
          set: {
            [`variants[${index}].reservedQuantity`]: Math.max(
              0,
...
async function autoFixOverselling(
