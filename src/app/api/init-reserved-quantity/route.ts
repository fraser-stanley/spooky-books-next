...
export async function POST() {
  try {
    console.log(
      "🔧 Initializing reservedQuantity fields for inventory management...",
    );
    ...
    console.log(
      `Found ${products.length} products needing reservedQuantity initialization`,
    );
    ...
    if (products.length === 0) {
      return NextResponse.json({
        success: true,
        message:
          "All products already have reservedQuantity fields initialized",
        productsUpdated: 0,
      });
    }
    ...
    for (const product of products) {
      ...
      if (product.variants && Array.isArray(product.variants)) {
        ...
        console.log(
          `✅ Updated ${updatedVariants.length} variants for: ${product.title}`,
        );
      }
    }
