...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      `🚀 Webhook received! Document type: ${body._type}, ID: ${body._id}`,
    );
    ...
    if (body._type === "product") {
      console.log(
        `🔄 Revalidating pages for product: ${body.title || body._id}`,
      );
      ...
      console.log(
        `✅ Product revalidation complete (with category sync): ${body.title || body._id}`,
      );
