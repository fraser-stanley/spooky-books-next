...
import {
  reserveStock,
  type StockOperation,
} from "@/lib/sanity/stock-operations";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!rateLimitResult.allowed) {
      console.log(
        `🚫 Rate limit exceeded for client: ${clientId} (${rateLimitResult.totalHits} attempts)`,
      );
      return createRateLimitResponse(rateLimitResult);
    }

    console.log(
      `✅ Rate limit check passed for client: ${clientId} (${rateLimitResult.totalHits}/5 attempts)`,
    );
    ...
    if (!reservationResult.success) {
      ...
      try {
        ...
        console.log(
          "🔄 Cancelled Stripe session due to stock reservation failure",
        );
      } catch (cancelError) {
    ...
    console.log(
      `📉 Rate limit count reduced for successful checkout: ${clientId}`,
    );
