...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!homepage) {
      return NextResponse.json({ error: 'No homepage document found' }, { status: 404 })
    }
    ...
    const contentBlocks = homepage.heroSections.map((section: any, index: number) => {
...
export async function GET() {
  return NextResponse.json({ 
    message: 'Homepage migration endpoint. Use POST to migrate content from heroSections to contentBlocks.' 
  })
}