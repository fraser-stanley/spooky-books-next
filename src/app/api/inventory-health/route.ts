...
export async function GET() {
  try {
    ...
    const enhancedHealthy =
      healthCheck.healthy &&
      oversellingIssues.issues.filter((i) => i.severity === "critical")
        .length === 0 &&
      raceConditionPatterns.patterns.filter((p) => p.severity === "high")
        .length === 0;
    ...
    return NextResponse.json({
      ...
      monitoring: {
        overselling: {
          ...
          criticalIssues: oversellingIssues.issues.filter(
        ...
        raceConditions: {
          ...
          highSeverityPatterns: raceConditionPatterns.patterns.filter(
            (p) => p.severity === "high",
          ).length,
      ...
      recommendations: enhancedHealthy
        ...
        : [
            ...(oversellingIssues.issues.length > 0
              ? ["Check overselling issues"]
              : []),
            ...(raceConditionPatterns.patterns.length > 0
              ? ["Monitor race condition patterns"]
              : []),
            ...healthCheck.issues,
          ],
    });
