...
async function handleCheckoutSessionCompleted(
  ...
  try {
    ...
    console.log(
      `Session ${session.id} completed with ${cartItems.length} items`,
    );
  } catch (error) {
...
async function handleCheckoutSessionExpired(session: Stripe.Checkout.Session) {
  ...
  try {
    ...
    const stockOperations: StockOperation[] = cartItems.map(
    ...
    if (result.success) {
      ...
      console.error(
        `Failed to release stock for expired session ${session.id}:`,
        result.errors,
      );
    }
    ...
    try {
      await fetch(
        `${process.env.NEXT_PUBLIC_SITE_URL}/api/autonomous-inventory`,
        {
          method: "POST",
...
async function handlePaymentSucceeded(paymentIntent: Stripe.PaymentIntent) {
  ...
  try {
    ...
    if (result.success) {
      console.log(
        `Successfully deducted stock for payment ${paymentIntent.id}`,
      );
      ...
      console.error(
        `Failed to deduct stock for payment ${paymentIntent.id}:`,
        result.errors,
      );
    ...
    console.error(
      `Error handling successful payment ${paymentIntent.id}:`,
      error,
    );
  }
}
...
async function handlePaymentFailed(paymentIntent: Stripe.PaymentIntent) {
  ...
  try {
    ...
    if (sessions.data.length === 0) {
      console.log(
        `No checkout session found for failed payment intent ${paymentIntent.id}`,
      );
      return;
    }
    ...
    if (result.success) {
      ...
      console.error(
        `Failed to release stock for failed payment ${paymentIntent.id}:`,
        result.errors,
      );
    }
...
async function cleanupReservation(sessionId: string) {
  try {
    ...
    console.error(
      `Failed to cleanup reservation for session ${sessionId}:`,
      error,
    );
