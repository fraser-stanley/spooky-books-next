...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Analyze results
    const criticalOversellingIssues = oversellingIssues.issues.filter(i => i.severity === 'critical')
    const highSeverityRaceConditions = raceConditionPatterns.patterns.filter(p => p.severity === 'high')
    
    const overallHealthy = healthCheck.healthy && 
    ...
    return NextResponse.json({
      ...
      recommendations: overallHealthy ? 
        ['System is healthy'] :
        [
          ...(criticalOversellingIssues.length > 0 ? ['⚠️ Critical overselling issues require immediate attention'] : []),
          ...(highSeverityRaceConditions.length > 0 ? ['⚠️ Race condition patterns detected'] : []),
          ...(healthCheck.issues || [])
        ]
    })
