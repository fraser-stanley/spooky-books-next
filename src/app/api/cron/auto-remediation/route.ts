  ...
  try {
    ...
    try {
      ...
      for (const product of productsWithReservations) {
        ...
        if (needsUpdate) {
          await sanityClient.patch(product._id).unset(['reservedQuantity']).commit()
    ...
    try {
      ...
      for (const product of productsWithNegativeStock) {
        ...
        if (product.variants) {
          product.variants.forEach((variant: {size: string, stockQuantity: number, reservedQuantity?: number}, index: number) => {
      ...
      for (const product of productsWithHighReserved) {
        ...
        if (product.variants) {
          product.variants.forEach((variant: {size: string, reservedQuantity: number}) => {
            ...
            transaction.patch(product._id, {
              set: { [`variants[_type == "variant" && size == "${variant.size}"][0].reservedQuantity`]: 0 }
            })
            remediationResults.highReservedQuantities.fixed += 1
          })
        }
    ...
    try {
      const totalFixes = remediationResults.staleReservations.fixed + 
      ...
      remediationResults.cacheRevalidation.error = error instanceof Error ? error.message : 'Unknown error'
      console.error('❌ Failed to revalidate cache:', error)
    }

    const duration = Date.now() - startTime
    const totalFixed = remediationResults.staleReservations.fixed + 
                      remediationResults.negativeStock.fixed + 
                      remediationResults.highReservedQuantities.fixed

    console.log(`${totalFixed > 0 ? '🔧' : '✅'} Auto-remediation completed: ${totalFixed} issues fixed (${duration}ms)`)
    ...
    return NextResponse.json({
      ...
      summary: {
        ...
        highReservedQuantitiesFixed: remediationResults.highReservedQuantities.fixed,
