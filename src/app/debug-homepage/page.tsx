      ...
      <div className="p-8 max-w-4xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <ul className="list-disc pl-5 space-y-1">
            ...
            <li>Old query heroSections: {oldHomepage.data?.heroSections?.length || 0}</li>
          </ul>
        </div>
          ...
          <div className="mb-8">
            ...
            {homepage.heroSections.map((section: any, index: number) => (
              <div key={index} className="border p-4 mb-2 rounded">
                <p><strong>Type:</strong> {section._type}</p>
                <p><strong>Title:</strong> {section.title}</p>
                <p><strong>Layout:</strong> {section.layout || 'N/A'}</p>
                <p><strong>Has Left Image:</strong> {section.leftImage ? '✅' : '❌'}</p>
                <p><strong>Has Right Image:</strong> {section.rightImage ? '✅' : '❌'}</p>
                <p><strong>Has Single Image:</strong> {section.image ? '✅' : '❌'}</p>
                <p><strong>Linked Product:</strong> {section.linkedProduct?.title || 'None'}</p>
              </div>
            ))}
          </div>
        )}
