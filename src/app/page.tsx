...
import type { SanityContentBlock, SanityHeroSection, SanityHeroPair, SanityHeroSingle } from "@/lib/sanity/types"
...
function ContentBlock({ block }: { block: SanityContentBlock }) {
  ...
  const getLink = () => {
    ...
    if (block.customLink) {
      return {
        ...
        isExternal: !block.customLink.url.startsWith('/') && !block.customLink.url.startsWith('#')
      }
    }
    return null
  }
  ...
  if (block.layout === 'full') {
    return <ContentBlockFullWidth block={block} link={link} linkProps={linkProps} />
  } else if (block.layout === 'two') {
    return <ContentBlockTwoColumn block={block} link={link} linkProps={linkProps} />
  } else {
    return <ContentBlockThreeColumn block={block} link={link} linkProps={linkProps} />
  }
}
