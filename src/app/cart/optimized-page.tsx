...
import {
  ...
  StockValidationOverlay,
} from "@/components/skeletons/checkout-skeleton";
        ...
        const outOfStock = stockValidation.stockChecks
          ...
          .map(
            (check: Record<string, unknown>) =>
              `${check.productTitle}${check.size ? ` (${check.size})` : ""}`,
          )
          .join(", ");

        alert(
          `Some items are no longer available: ${outOfStock}. Please update your cart.`,
        );
